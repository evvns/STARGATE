...
def stargate(content: str, key: int) -> str:
    ...
    _names_ = ["_eval", "_exec", "_byte", "_bytes", "_bit", "_bits", "_system", "_encode", "_decode", "_delete", "_exit", "_rasputin", "_stargate"]
    ...
    _final_content_ = fr"""class stargate():
...
stargate(_n1_=False,_n2_=False,_sparkle='''{content}''')""".strip().replace("_n1_",n_1.removeprefix("self.")).replace("_n2_",n_2.removeprefix("self.")).replace("_n3_",n_3.removeprefix("self.")).replace("_n4_",n_4.removeprefix("self.")).replace("_n5_",n_5).replace("_n6_",n_6).replace("_n7_",n_7).replace("_n8_",n_8).replace("_n9_",n_9.removeprefix("self.")).replace("_n10_",n_10.removeprefix("self.")).replace("_n12_",n_12.removeprefix("self."))
    return _final_content_



System.Clear()
System.Title("stargate")
System.Size(140, 45)
...
STARGATE - HIGH FIDELITY PYTHON OBFUSCATOR.
copyright 2022 gnaa.world
...
def main():
    ...
    _file = Write.Input("[STARGATE] Drag your Python file -> ", Colors.red_to_yellow, interval=0.005)
    ...
    content = stargate(content=content, key=key)
